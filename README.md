...
- `POST /deployments/:deployment/duplicate` - Duplicate a deployment into a new DRAFT (same configuration, the active revision as revision 1); optional body `{ "name": "...", "autostart": true }` names it (default "<source name> (copy)") and starts it immediately
...
- `PATCH /deployments/:deployment/update-startup-timeout` - Update the startup timeout (INFINITE deployments only)

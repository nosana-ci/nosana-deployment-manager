...
|---|---|---|
| `lost` | frpc / the node died — control connection dropped without a goodbye | Schedule a **grace-delayed stop** (default 60s). If the tunnel re-registers inside the grace, the stop is cancelled. Otherwise the job stops and the infinite strategy posts a replacement. |
| `unhealthy` | Backend service failed its health check — frpc still connected, the node is fine | **Same as `lost`** — to the job poster both mean the service is unreachable. The same grace covers a backend restarting via its container restart policy; if it recovers, the re-registration cancels the stop. The distinct reason is kept in the endpoint status and event message for diagnosis. |
...
|---|---|---|
| `FRPS_INTERNAL_ADDRESS` | *(empty — disabled)* | Cluster-internal FRPS host **including port**, e.g. `frps.frps.svc.cluster.local:7501`. Empty disables watching entirely. |
...
| `FRPS_INTERNAL_USE_TLS` | `false` | Scheme for the internal address. |
...
**New collection — `frps_endpoint_status`.** One document per `(job, opId)`: `state: up|down`, `reason`, `last_change`. Written from every lifecycle event, including graceful ones — queryable op-level tunnel truth for the dashboard. Migration `17` adds the unique `(job, opId)` index.

**Deployment events** (user-visible in the existing events feed):

- `FRPS_TUNNEL_LOST` — job unreachable (tunnel lost or health check failing; the message states which); it will be stopped and replaced in *N*s unless it recovers.
...
2. **Roll the new frpc image across the node fleet.** Old frpc exits on SIGTERM without a goodbye, so with old frpc **every clean stop reports `lost`**, and a dead backend reports `graceful`.
...
- **`unhealthy` requires a health check.** frpc only probes the backend when the job definition declares an HTTP health check (`type: http`, `GET`, expected 200). Without one, a dead backend behind a live tunnel produces no signal at all.
...
5. With a health-checked deployment, kill the backend only: expect `FRPS_TUNNEL_LOST` (message citing health checks), the endpoint row `down/unhealthy`, and a stop + replacement ~60s later — or `FRPS_TUNNEL_RECOVERED` and no stop if the backend restarts inside the grace.

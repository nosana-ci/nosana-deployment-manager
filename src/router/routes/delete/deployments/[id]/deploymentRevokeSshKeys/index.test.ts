...
import { addSchemas, DeploymentRevokeSshKeysSchema } from "../../../../../schema/index.schema.js";
import { deploymentRevokeSshKeysHandler } from "./index.js";
...
async function buildServer(): Promise<FastifyInstance> {
  ...
  server.delete(
    ...
    { schema: DeploymentRevokeSshKeysSchema, preHandler: [setDeployment] },
...
describe("DELETE /deployments/:deployment/ssh-keys", () => {
  let server: FastifyInstance;

  beforeEach(async () => {
    spawned.length = 0;
    deploymentDoc.confidential = false;
    deploymentDoc.ssh_public_keys = [KEY_A, KEY_B];
    pin.mockClear();
  ...
  const revoke = (body: unknown) =>
    server.inject({ method: "DELETE", url: `/deployments/${DEPLOYMENT}/ssh-keys`, headers: AUTH_HEADERS, payload: body });

  it("removes a key and revokes it on every running job", async () => {
    const jobA = "A".repeat(44);
    runningJobs([{ job: jobA, node: NODE }]);

    // Match by material even with a different comment.
    const res = await revoke({ public_keys: [`ssh-ed25519 ${KEY_A.split(" ")[1]} whatever`] });

    expect(res.statusCode).toBe(200);
    expect(res.json().public_keys).toEqual([KEY_B]);
    expect(res.json().jobs).toEqual([{ job: jobA, node: NODE, status: "revoked" }]);
    expect(pin).toHaveBeenCalledWith({ ...DEFINITION, ssh: { public_keys: [KEY_B] } });
    expect(db.deployments.updateOne).toHaveBeenCalledWith(
      { id: { $eq: DEPLOYMENT }, owner: { $eq: OWNER } },
      { $set: { ssh_public_keys: [KEY_B], updated_at: expect.any(Date) } }
    );
    expect(spawned).toEqual([
      { vault: VAULT_KEY, operation: "revoke", public_keys: [KEY_A], jobs: [{ job: jobA, node: NODE }] },
    ]);
  ...
  it("unsets the field when the last key is revoked", async () => {
    deploymentDoc.ssh_public_keys = [KEY_A];
    runningJobs([{ job: "A".repeat(44), node: NODE }]);

    const res = await revoke({ public_keys: [KEY_A] });

    expect(res.statusCode).toBe(200);
    expect(res.json().public_keys).toEqual([]);
    expect(pin).toHaveBeenCalledWith(DEFINITION); // bare re-pin, ssh removed
    expect(db.deployments.updateOne).toHaveBeenCalledWith(
      expect.anything(),
      { $set: { updated_at: expect.any(Date) }, $unset: { ssh_public_keys: "" } }
    );
    expect(spawned).toEqual([
      { vault: VAULT_KEY, operation: "revoke", public_keys: [KEY_A], jobs: [{ job: "A".repeat(44), node: NODE }] },
    ]);
  });

  it("pushes nothing when none of the requested keys are present", async () => {
    runningJobs([{ job: "A".repeat(44), node: NODE }]);
    const unknownKey = "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIB0XqCL4vLIsYRvd5VmtbOJ8IEKDJpjaVWQ5lmxWVTq7 c";

    const res = await revoke({ public_keys: [unknownKey] });

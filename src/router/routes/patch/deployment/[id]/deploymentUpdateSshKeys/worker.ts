...
import { pushSshKeysToNode } from "../../../../../../ssh/index.js";
 ...
 * Runs in a worker because that is where the vault key is decrypted, needed to
 * sign the job-owner `authorization` header (the same signer behind
 * `GET /deployments/:id/header`, timestamped) that authenticates each authorize
 * request against `job.project`. Its content isn't checked node-side, so one
 * header serves every job in the rotation.
 */
try {
  ...
  const results: JobSshKeysResult[] = await Promise.all(
    jobs.map(async ({ job, node }): Promise<JobSshKeysResult> => {
      try {
        const result = await pushSshKeysToNode({ node, job, public_keys, authHeader });
        return result.ok

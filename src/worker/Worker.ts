...
type VaultWorkerOptions<T extends VaultWorkerData> = WorkerOptions & {
  workerData: T;
};

export class VaultWorker<
  T extends VaultWorkerData = VaultWorkerData,
> extends NodeWorker {
...
export async function prepareWorker<
  T extends VaultWorkerData = VaultWorkerData,
>(workerData: T): Promise<T & { kit: NosanaClient; useNosanaApiKey: boolean }> {
  try {
